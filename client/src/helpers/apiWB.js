...
function findMissing(array, array2) {
 let result = [];
 let fullArray = [];
 //console.log(range)
 ...
 const res = array2.map((el, index) => {
   for (
     let i = range[index].date_from.day;
     i <= range[index].date_to.day;
     i++
   ) {
     // console.log(i, 'i')
     if (!el.includes(i)) {
...
function findMissing1(array, array2) {
 let result = [];
...
const reqBodyFormated = (reqBody) => {
 ...
 if (month1 === month2) {
   ...
   if (differenceBetweenMonths >= 1) {
     //* если диапазон больше одного месяца, то ищем кол дней в промежутке между ними
 
     let rangeArray = [];
 
     for (let i = month1; i <= month2; i++) {
       //* ищем месяца которые находятся в промежутке
...
const mainFunction = (obj, reqBody) => {
  ...
  let newObj = sortedArray.map((el, index, array) => {
    ...
    el.map((e, index) => {
      ...
      res.push({
        ...
        month: month,
        day: day,
        doc_type_name: e.doc_type_name,
        retail_amount: e.retail_amount,
        delivery_rub: e.delivery_rub,
      });
    });
    return res;
  });
    ...
    FINALARRAY = returnesArray2.map((el, index) => {
      ...
      if (!missingDays[index].includes(el[index].day)) {
        ...
        if (missingDays[index].length !== 0) {
          ...
          OBJ = {
            year: el[index].year,
            month: el[index].month,
            day: missingDays[index],
            retail_amount: 0,
            delivery_rub: 0,
          };
          result.push(OBJ);
  ...
  const FINALARRAY3 = FINALARRAY2.map((elem, index) => {
    ...
    for (let el of elem) {
      if (el.month < 10 && el.day < 10) {
        ...
        result.push({
          date: `${el.year}-${month}-${day}`,
          retail_amount: el.retail_amount,
          delivery_rub: el.delivery_rub,
        });
      } else if (el.month < 10 && el.day >= 10) {
        let month = `0${el.month}`;
  
        result.push({
          date: `${el.year}-${month}-${el.day}`,
          retail_amount: el.retail_amount,
          delivery_rub: el.delivery_rub,
        });
      } else if (el.month >= 10 && el.day < 10) {
...
const returnsFunction = (obj, reqBody) => {
 range = reqBodyFormated(reqBody);
 //console.log(range, 'rangeRETURNS')
 ...
 //console.log(sortedArray, 'sortedArrayRETURNS')
 ...
 // console.log(newObj, 'newObjRETURNS'); // массив с разбитой датой
 
 //* здесь мы выбираем только возвраты
 let group1 = newObj[1].reduce((r, a) => {
   r[a.month] = [...(r[a.month] || []), a];
   return r;
 }, {});
 
 let returnesArray = Object.values(group1);
 // console.log(returnesArray, 'returnesArray')
 ...
 let array2 = returnesArray.map((el, index, array) => {
   ...
   let array2 = Object.values(group2);
   //console.log(array2, 'array2RETURNS')
 
   for (let elem of array2) {
   //  console.log(elem[index].return_amount);
     total1 = elem.reduce((acc, el) => el.quantity + acc, 0);
     finalArray.push({
       year: elem[0].year,
       month: elem[0].month,
       day: elem[0].day,
 ...
 ///console.log(returnesArray2, 'returnesArray2')
 ...
 //console.log(numberOfDays, 'numberOfDaysRETURNS')
 //console.log(findMissing2(returnesArray2, numberOfDays), 'findMissing2')
 ...
 if (range[0].nMonths >= 1) {
   missingDays = findMissing21(returnesArray2, numberOfDays);
   // console.log(missingDays, 'missingDAYYYYS')
   FINALARRAY = returnesArray2.map((el, index) => {
     let OBJ = {};
     let result = [];
     if (!missingDays[index].includes(el[index].day)) {
   ...
   missingDays = findMissing2(returnesArray2, numberOfDays);
   FINALARRAY = returnesArray2.map((el, index) => {
     let OBJ = {};
     let result = [];
     for (let elem of el) {
 ...
 /*const missingDays = findMissing2(returnesArray2, numberOfDays)
 
 
 const FINALARRAY = returnesArray2.map((el, index) => {
     let OBJ = {}
     let result = []
     for (let elem of el) {
     if (!missingDays.includes(elem.day)){
 ...
 console.log(FINALARRAY3, "<===== WB RETURNS DATA");
 return FINALARRAY3;
};
 
function findMissing2(array, array2) {
 let result = [];
...
function findMissing21(array, array2) {
 let result = [];
 let fullArray = [];
 //let ARRAY = []
 //console.log(range)
...
const ordersFunction = (obj, reqBody) => {
 range = reqBodyFormated(reqBody);
 //console.log(range, 'range')
 ...
 const sortedArray = array.map((el) => {
   el.sort(
     (prev, next) => Date.parse(prev.order_dt) - Date.parse(next.order_dt)
   );
   return el;
 });
 ...
 let returnesArray = Object.values(group1);
 // console.log(returnesArray, 'ordersArray')
 ...
 //console.log(returnesArray2, 'ordersArray2')
 ...
 const RES = FINALARRAY3.reduce((r, e) => (r.push(...e), r), []);
 console.log(RES, "<===== ORDERS WB DATA");
 return RES;
};
 
export { mainFunction, returnsFunction, ordersFunction };
